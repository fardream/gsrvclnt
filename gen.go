...
func GenForFile(g *protogen.GeneratedFile, f *protogen.File) {
	for _, s := range f.Services {
		...
		g.P("contextUpdaters []func(", contextContext, ") (", contextContext, ", error)")
		g.P("}")
		g.P("var _ ", s.GoName, "Client = (*", stypename, ")(nil)")

		g.P("func New", s.GoName, "SrvClient(server ", s.GoName, "Server, contextUpdaters ...func(", contextContext, ") (", contextContext, ", error)) ", s.GoName, "Client {")
		g.P("return &", stypename, "{Server: server, contextUpdaters: contextUpdaters}")
		g.P("}")
		...
		for _, m := range s.Methods {
			...
			case m.Desc.IsStreamingClient() && m.Desc.IsStreamingServer():
				...
				clientype := clientname(s, m, g)
				g.P("for _, u := range client.contextUpdaters {")
				g.P("var err error")
				g.P("ctx, err = u(ctx)")
				g.P("if err != nil {")
				g.P("return nil, err")
...
func genClientStream(s *protogen.Service, m *protogen.Method, g *protogen.GeneratedFile) {
	...
	if m.Desc.IsStreamingClient() {
		...
		g.P("if client.errfromsrv != nil {")
		g.P("return client.errfromsrv")
		g.P("}")
